             ...
             real old[MAXFORCEPARAM], int comb, double reppow)
{
    ...
    switch (ftype)
    {
        ...
        case F_PUMBRELLA:
            ...
            break;
        case F_TUMBRELLA:
            newparam->tumbrella.amp    = old[0];
            newparam->tumbrella.mu     = old[1];
            newparam->tumbrella.kappa  = old[2];
            newparam->tumbrella.cut    = old[3];
            break ;
#endif // SBM_RUMB
#ifdef SBM
        case F_TWOANGLE:
	    if ( old[2] >= old[0] )
   	    {
  	        newparam->twoangle.r1=old[0]*(DEG2RAD);
                newparam->twoangle.k1=old[1];
     	        newparam->twoangle.r2=old[2]*(DEG2RAD);
  	        newparam->twoangle.k2=old[3];
   	    }
   	    else
   	    {
  	        newparam->twoangle.r1=old[2]*(DEG2RAD);
   	        newparam->twoangle.k1=old[3];
   	        newparam->twoangle.r2=old[0]*(DEG2RAD);
   	        newparam->twoangle.k2=old[1];
   	    }
 	        newparam->twoangle.bar=old[4];
  	    break;
        case F_TWODIHS:
	    newparam->twodihs.amp=old[0];
	    tmp1 = old[1] ;
	    while(tmp1>360) tmp1-=360 ;
	    while(tmp1<0)   tmp1+=360 ;
	    tmp2 = old[2] ;
	    while(tmp2>360) tmp2-=360 ;
	    while(tmp2<0)   tmp2+=360 ;
	    if (tmp1<=180 && tmp2>180) tmp1+=360 ;
	    if (tmp1>=180 && tmp2<180) tmp1-=360 ;
	    if (tmp1<180 && tmp2>=180) tmp2-=360 ;
	    if (tmp1>180 && tmp2<=180) tmp2+=360 ;
	    newparam->twodihs.mu1=min(tmp1,tmp2);
	    newparam->twodihs.mu2=max(tmp1,tmp2);
	 // mu1 < mu2 and both on the same side of 180
	    break;
